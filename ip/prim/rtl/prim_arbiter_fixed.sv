...
//
// NumReq:1 fixed priority arbiter module (index 0 has highest prio)
//
// Verilog parameter
...
module prim_arbiter_fixed #(
  ...
  if (NumReq == 1) begin : gen_degenerate_case
    ...
    // a full binary tree with NumReq levels has 2**NumReq + 2**NumReq-1 nodes
    logic [2**(IdxW+1)-2:0] req_tree;
    logic [2**(IdxW+1)-2:0] gnt_tree;
    logic                   [2**(IdxW+1)-2:0][IdxW-1:0] idx_tree;
    logic                   [2**(IdxW+1)-2:0][DataWidth-1:0]   data_tree;
    ...
    for (genvar level = 0; level < IdxW+1; level++) begin : gen_tree
      ...
      for (genvar offset = 0; offset < 2**level; offset++) begin : gen_level
        ...
        if (level == IdxW) begin : gen_leafs
          if (offset < NumReq) begin : gen_assign
            // forward path
  ...
  `ASSERT(Priority_A, |req_i |-> req_i[idx_o] && (((NumReq'(1'b1) << idx_o) - 1'b1) & req_i) == '0)
