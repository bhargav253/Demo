...
class cfg_const:
    ...
    DMA_CSRs = {}
    #-------------------------------> Addr     Mask       RW
    DMA_CSRs['CONTROL']        = (0x0000, 0x3FF,      1)
    DMA_CSRs['STATUS']         = (0x0004, 0xCAFE,     0)
    DMA_CSRs['ERROR_ADDR']     = (0x0028, 0x0,        0)
    DMA_CSRs['ERROR_STATS']    = (0x002c, 0x0,        0)
    NUM_DESC       = 2
    PER_DESC_CSRS  = 4
    CSR_ADDR_ALIG  = 4
    BASE_ADDR_DESC = 0x08
    for i in range(0, NUM_DESC):
        DMA_CSRs['DESC_SRC_ADDR_'      +str(i)] = (BASE_ADDR_DESC+(0*(NUM_DESC*8))+(i*CSR_ADDR_ALIG), 0xFFFFFFFF, 1)
        DMA_CSRs['DESC_DST_ADDR_'      +str(i)] = (BASE_ADDR_DESC+(1*(NUM_DESC*8))+(i*CSR_ADDR_ALIG), 0xFFFFFFFF, 1)
        DMA_CSRs['DESC_NUM_BYTES_'     +str(i)] = (BASE_ADDR_DESC+(2*(NUM_DESC*8))+(i*CSR_ADDR_ALIG), 0xFFFFFFFF, 1)
        DMA_CSRs['DESC_CFG_WRITE_MODE_'+str(i)] = (BASE_ADDR_DESC+(3*(NUM_DESC*8))+(i*CSR_ADDR_ALIG), 0x7,        1)
        DMA_CSRs['DESC_CFG_READ_MODE_' +str(i)] = (BASE_ADDR_DESC+(3*(NUM_DESC*8))+(i*CSR_ADDR_ALIG), 0x7,        1)
        DMA_CSRs['DESC_CFG_ENABLE_'    +str(i)] = (BASE_ADDR_DESC+(3*(NUM_DESC*8))+(i*CSR_ADDR_ALIG), 0x7,        1)
    ################### End Configure ####################
