...
module axil_adaptor 
  import intf_pkg::*;
   (/*AUTOARG*/
   // Outputs
   axil_waddr_t, axi_awreq_o, axi_awvalid_o, axil_wdata_t, axi_wreq_o,
   axi_wvalid_o, axi_bready_o, axil_raddr_t, axi_arreq_o,
   axi_arvalid_o, axi_rready_o, intf_req_accept_o, intf_rsp_val_o,
   intf_rsp_t, intf_rsp_o,
   // Inputs
   clk_i, rst_ni, axi_awready_i, axi_wready_i, axil_bresp_t,
   axi_brsp_i, axi_bvalid_i, axi_arready_i, axil_rdata_t, axi_rrsp_i,
   axi_rvalid_i, intf_req_t, intf_req_i
   );
   ...
   always_comb begin
	 ...
	 rsp_skid_in_data  = {intf_req_i.req_tag,intf_req_i.ren ? XCTN_RD : XCTN_WR};
      end
   end
   ...
   /*
    ...
    .i_data_i  (skid_in_data),
    .i_valid_i (skid_in_valid),
    .i_ready_o (skid_in_ready),
    .e_data_o  (skid_out_data),
    .e_valid_o (skid_out_valid),
    .e_ready_i (skid_out_ready),   
    );
    */
   ...
   u_req_slice (/*AUTOINST*/
		// Outputs
		.i_ready_o		(skid_in_ready),	 // Templated
		.e_data_o		(skid_out_data),	 // Templated
		.e_valid_o		(skid_out_valid),	 // Templated
		// Inputs
		.clk_i,
		.rst_ni,
		.i_data_i		(skid_in_data),		 // Templated
		.i_valid_i		(skid_in_valid),	 // Templated
		.e_ready_i		(skid_out_ready));	 // Templated


   assign rsp_skid_out_ready = rsp_skid_out_valid & (rsp_skid_out_data[0] == XCTN_RD) ? axi_rvalid_i :
			       rsp_skid_out_valid & (rsp_skid_out_data[0] == XCTN_WR) ? axi_bvalid_i : 1'b0;   
   ...
   always_comb begin
      ...
      if((rsp_skid_out_data[0] == XCTN_WR) & rsp_skid_out_valid) begin
	 ...
	 rsp_skid_out_ready  = axi_bvalid_i;	 
      ...
      else if((rsp_skid_out_data[0] == XCTN_RD) & rsp_skid_out_valid) begin
