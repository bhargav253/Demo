...
def _resolve_ip_iface(ip_path: Path) -> str:
    """Return a short bus interface string (e.g., tlul/apb/ahb) for the IP."""
    try:
        data = hjson.load(ip_path.open())
        interfaces = data.get("bus_interfaces", [])
        if interfaces:
            proto = interfaces[0].get("protocol", "")
            return proto or "-"
    except Exception:
        pass
    return "-"
...
def parse_top(top_path: Path) -> Tuple[List[Tuple[str, int, str, str]], List[Tuple[str, int, int]]]:
    ...
    for m in data.get("module", []):
        ...
        ip_path = (top_path.parent / ip_hjson) if ip_hjson else None
        iface = _resolve_ip_iface(ip_path) if ip_path and ip_path.exists() else "-"
        mods.append((name, _get_base_addr(m, f"module {name}"), target, iface))
