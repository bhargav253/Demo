...
<%
  ...
  dynamic_racl_support = False
  static_racl_support = False
  racl_support = False
  is_apb = protocol == 'apb'
  is_ahb = protocol == 'ahb'
  is_tlul = protocol == 'tlul'
  if is_ahb:
    raise RuntimeError("AHB interface not yet supported in this template")
  if is_apb and num_wins != 0:
    raise RuntimeError("APB interface does not support windows in this template")
  if is_apb and rb.async_if:
    raise RuntimeError("APB asynchronous interfaces are not supported")
  if racl_support and not is_tlul:
    raise RuntimeError("RACL is only available for TL-UL device interfaces")
  expose_reg_if = getattr(block, 'expose_reg_if', False)
...
%>\
  ...
  logic bus_req_valid;
  % if is_tlul:
  assign bus_req_valid = tl_i.a_valid;
  % elif is_apb:
  assign bus_req_valid = apb_req_i.psel;
  % else:
  assign bus_req_valid = 1'b0;
  % endif
  assign reg_busy = (reg_busy_sel | shadow_busy) & bus_req_valid;
  always_comb begin
    reg_busy_sel = '0;
    unique case (1'b1)
