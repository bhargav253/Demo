...
class BusInterfaces:
    ...
    def __init__(self, has_unnamed_host: bool, named_hosts: List[str],
                 ...
                 device_protocols: Dict[Optional[str], str]):
    ...
    def from_raw(raw: object, where: str) -> 'BusInterfaces':
        ...
        for idx, raw_entry in enumerate(check_list(raw, where)):
            ...
            if protocol not in ['tlul', 'apb', 'ahb']:
                raise ValueError(
            ...
            if direction == 'host':
                ...
                if name is None:
                    ...
                    device_protocols[None] = protocol
                else:
                ...
                device_hier_paths[name] = hier_path if hier_path is not None else 'u_reg'
                device_protocols[name] = protocol
        ...
        return BusInterfaces(has_unnamed_host, named_hosts, host_async,
                             ...
                             device_hier_paths, device_protocols)

    def get_device_protocol(self, name: Optional[str]) -> str:
        if name in self.device_protocols:
            return self.device_protocols[name]
        if self.has_unnamed_device and name is None:
            return self.device_protocols.get(None, 'tlul')
        # default to tlul
        return 'tlul'
    ...
    @staticmethod
    def _if_dict(is_host: bool, name: Optional[str],
    ...
    def as_dicts(self) -> List[Dict[str, object]]:
        ...
        for is_host, name in self._interfaces():
            protocol = 'tlul' if is_host else self.get_device_protocol(name)
            ret.append(BusInterfaces._if_dict(is_host, name, protocol))
        return ret
    ...
    def get_port_name(self, is_host: bool, name: Optional[str]) -> str:
        if is_host:
            ...
            protocol = self.get_device_protocol(name)
            if protocol == 'tlul':
                suffix = 'tl_d' if self.has_host() else 'tl'
            else:
                suffix = protocol

        return suffix if name is None else f'{name}_{suffix}'
    ...
    # inter-signal helpers removed in lightweight config
